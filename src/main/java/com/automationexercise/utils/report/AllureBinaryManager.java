...
public class AllureBinaryManager {
    ...
    public static void downloadAndExtract() {
        try {
            ...
            Path extractionDir = Paths.get(com.automationexercices.utils.report.AllureConstants.EXTRACTION_DIR.toString(), "allure-" + version);
            ...
            if (!OSUtils.getCurrentOS().equals(OSUtils.OS.WINDOWS)) {
                TerminalUtils.executeTerminalCommand("chmod", "u+x", com.automationexercices.utils.report.AllureConstants.USER_DIR.toString());
            }
            ...
            Files.deleteIfExists(Files.list(com.automationexercices.utils.report.AllureConstants.EXTRACTION_DIR).filter(p -> p.toString().endsWith(".zip")).findFirst().orElseThrow());
    ...
    public static Path getExecutable() {
        ...
        Path binaryPath = Paths.get(com.automationexercices.utils.report.AllureConstants.EXTRACTION_DIR.toString(), "allure-" + version, "bin", "allure");
    ...
    private static Path downloadZip(String version)   {
        try {
            ...
            String url = com.automationexercices.utils.report.AllureConstants.ALLURE_ZIP_BASE_URL + version + "/allure-commandline-" + version + ".zip";
            //C:\Users\hussi\.m2\repository\allure
            Path zipFile = Paths.get(com.automationexercices.utils.report.AllureConstants.EXTRACTION_DIR.toString(), "allure-" + version + ".zip");
            if (!Files.exists(zipFile)) {
                Files.createDirectories(com.automationexercices.utils.report.AllureConstants.EXTRACTION_DIR);
    ...
    private static void extractZip(Path zipPath)   {
        try (ZipInputStream zipInputStream = new ZipInputStream(Files.newInputStream(zipPath))) {
            ...
            while ((entry = zipInputStream.getNextEntry()) != null) {
                Path filePath = Paths.get(com.automationexercices.utils.report.AllureConstants.EXTRACTION_DIR.toString(), File.separator, entry.getName());
